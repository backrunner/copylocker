 ...
 */
import { decode, encode, mapGet } from '../cbor.js';
...
function writeHeader(id, kind, payload) {
...
export function encodeResponse(id, status, payload) {
...
export function encodeCoreError(code) {
    const payload = new Uint8Array(4);
    new DataView(payload.buffer).setUint32(0, code >>> 0, true);
    return payload;
}
export function decodeCoreError(payload) {
    if (payload.byteLength !== 4)
        throw new TypeError('worker protocol: bad error payload');
...
export function encodeInit(init) {
    ...
    if (init.glueUrl !== undefined)
        map.set(3, init.glueUrl);
    return encode(map);
}
export function decodeInit(payload) {
    ...
    return { wasmBytes, cfg, glueUrl: glueUrl };
}
