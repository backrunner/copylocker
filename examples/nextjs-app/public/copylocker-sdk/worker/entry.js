...
import { OP_INIT, OP_STEP, STATUS_CORE_ERROR, STATUS_HOST_ERROR, STATUS_OK, decodeInit, decodeRequest, encodeCoreError, encodeResponse, messageBytes, } from './protocol.js';
 ...
 * concern.
 */
export function createMessageHandler(factory) {
    let session = null;
    return async (frame) => {
        let id = 0;
        try {
                ...
                case OP_STEP: {
                    ...
                    try {
                        ...
                        if (isErrorCode(error))
                            return encodeResponse(id, STATUS_CORE_ERROR, encodeCoreError(error));
                        throw error;
                    }
                }
                default:
