/**
 * Minimal canonical (deterministic) CBOR codec, dependency-free.
 *
 * Vendored verbatim from `packages/web/src/cbor.ts` so the sealed-asset
 * containers emitted here are byte-identical to what `@copylocker/web`
 * decodes at runtime. Keep the two files in sync; the cross-tests in
 * `test/container.test.ts` fail if they drift apart.
 *
