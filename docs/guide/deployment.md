...
- Do not place Root keys, Epoch keys, webhook secrets, Admin tokens, or pepper values in the
  ...
  [Runbook → Key and credential inventory](../operations/runbook#key-and-credential-inventory).
...
condition — see [SLOs & Alerting](../operations/slo#alert-thresholds).
...
the [go-live checklist](./protection-levels#go-live-checklist).
