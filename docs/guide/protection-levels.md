...
::: details Why the internal design sketch differs
The internal module doc shows string codes (`e.code === 'NOT_ENTITLED'`). The shipped web SDK
instead throws typed errors carrying a numeric `code` (NFR-SEC-011: no greppable feature
strings). The classes above are the real contract.
:::
...
- [ ] The production signer is not `local` (see [signer modes](./web-sdk#build-time-integration-unplugin-guard-seal))
